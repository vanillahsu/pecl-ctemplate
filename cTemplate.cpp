/*
  ...
  | PHP Version 5                                                        |
  +----------------------------------------------------------------------+
  | Copyright (c) 2006 - 2012 San Tai (Vanilla) Hsu                      |
  +----------------------------------------------------------------------+
...
static zend_object_value cTemplateTpl_object_new_ex(
        zend_class_entry *ce, php_cTemplateTpl **ptr TSRMLS_DC);
static zend_object_value cTemplateTpl_object_clone(
        zval *this_ptr TSRMLS_DC);
static zend_object_value cTemplateDict_object_new(
        zend_class_entry *ce TSRMLS_DC);
static zend_object_value cTemplateDict_object_new_ex(
        zend_class_entry *ce, php_cTemplateDict **ptr TSRMLS_DC);
static zend_object_value cTemplateDict_object_clone(
        zval *this_ptr TSRMLS_DC);
...
PHP_MINIT_FUNCTION(cTemplate) {
    REGISTER_LONG_CONSTANT("DO_NOT_STRIP", DO_NOT_STRIP,
            CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("STRIP_BLANK_LINES", STRIP_BLANK_LINES,
            CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("STRIP_WHITESPACE", STRIP_WHITESPACE,
            CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("NUM_STRIPS", NUM_STRIPS,
            CONST_CS | CONST_PERSISTENT);
    ...
    REGISTER_LONG_CONSTANT("VALIDATE_URL_AND_HTML_ESCAPE", 5,
            CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("VALIDATE_URL_AND_JAVASCRIPT_ESCAPE", 6,
            CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("VALIDATE_URL_AND_CSS_ESCAPE", 7,
            CONST_CS | CONST_PERSISTENT);
    ...
    REGISTER_LONG_CONSTANT("JAVASCRIPT_NUMBER", 10,
            CONST_CS | CONST_PERSISTENT);
...
PHP_METHOD(cTemplateTpl, Expand) {
    ...
    if (Z_TYPE_P(val) == IS_OBJECT) {
        ...
        if (dict->obj == NULL) {
            zend_throw_exception(zend_exception_get_default(TSRMLS_C),
                    "Template dictionary object not exist", 0 TSRMLS_CC);
            return;
        }
...
PHP_METHOD(cTemplateDict, SetArray) {
    ...
    for (zend_hash_internal_pointer_reset(ar);
        ...
        if (type != HASH_KEY_IS_STRING) {
            zend_throw_exception(zend_exception_get_default(TSRMLS_C),
                    "all element keys must be string, not index", 0 TSRMLS_CC);
            return;
        }
...
PHP_METHOD(cTemplateDict, Set) {
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sz|s",
                &key, &key_len, &val, &sec, &sec_len) == FAILURE)
        RETURN_FALSE;

    if (Z_TYPE_P(val) == IS_STRING) {
        if (sec != NULL) {
            if (dict->obj->is_root) {
                dict->obj->d.SetValueAndShowSection(key,
                        Z_STRVAL_P(val), sec);
            } else {
                dict->obj->p->SetValueAndShowSection(key,
                        Z_STRVAL_P(val), sec);
            }
        } else {
            if (dict->obj->is_root)
...
static zend_object_value cTemplateTpl_object_new_ex(zend_class_entry *ce,
    ...
    object_properties_init(&tpl->std, ce);
#else
...
#if PHP_VERSION_ID > 50399
    object_properties_init(&dict->std, ce);
#else
...
static zend_object_value cTemplateTpl_object_clone(zval *this_ptr TSRMLS_DC) {
    ...
    zend_object_value new_ov = cTemplateTpl_object_new_ex(old_obj->std.ce,
            &new_obj TSRMLS_CC);
...
static zend_object_value cTemplateDict_object_clone(zval *this_ptr TSRMLS_DC) {
    ...
    zend_object_value new_ov = cTemplateDict_object_new_ex(old_obj->std.ce,
            &new_obj TSRMLS_CC);
...
void cTemplateTpl_init(TSRMLS_D) {
    ...
    cTemplateTpl_ce = zend_register_internal_class_ex(&ce, NULL,
            NULL TSRMLS_CC);
    memcpy(&cTemplateTpl_object_handlers, zend_get_std_object_handlers(),
...
void cTemplateDict_init(TSRMLS_D) {
    ...
    cTemplateDict_ce = zend_register_internal_class_ex(&ce, NULL,
            NULL TSRMLS_CC);
    memcpy(&cTemplateDict_object_handlers, zend_get_std_object_handlers(),
            sizeof(zend_object_handlers));
